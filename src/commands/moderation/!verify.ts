    ...
    try {
      if (isValidSnowflake(args[0]) === false) {
        let errorMsg = await message.reply({
          content: `${args[0]} is not a valid message id. Please try again <t:${Date.now() + 5000}:R>.`
        });
      ...
      if (message.channel.isTextBased()) {
        ...
        const chan = message.guild?.channels.cache.get('744945603313795306') as TextChannel;
          ...
          message.guild?.channels.cache.each(async (channel: Channel) => {
            if (channel.isTextBased() && channel === chan) {
              ...
              if (messages.has(args[0])) {
                ...
                const member = (await message.guild?.members.fetch())?.get(memberId)!;
                ...
                const welcome = (await message.guild?.channels.fetch(guild?.welcomeC!)) as TextChannel;
